 ...
 *
 * File    : simulink.c                                  $Revision: 1.89.4.30 $
 * Abstract:
 ...
 * Returns:
 *      A int_T: current input port width
 */
 ...
 * Returns:
 *      A int_T: current output port width
 */
 ...
 * Returns:
 *      A int_T: the number of (branched) function-call destinations that are
...
"that it has direct feedthrough using "
"ssSetDirectFeedThrough(S,1). %s";

const char _ssBadLevel1UAccessMsg2[] =
"Please update the S-function to have direct feedthrough or "
...
const char _ssBadLevel2DirectFeedMsg[] =
  ...
  "invalid port index will cause a segmentation violation. ";

const char _ssBadLevel2DirectFeedSolutionMsg[] =
 "Please verify that "
...
const char _ssBadLevel2ZeroDirectFeedMsg2[] =
"Please verify that the ssSetInputPortDirectFeedThrough "
...
static void _ssFatalError(const SimStruct *S, char *NagMsg)
{
    ...
    mexCallMATLAB(0, NULL, (int) (sizeof(prhs)/sizeof(prhs[0])), prhs, "errordlg");
...
InputPtrsType _ssGetInputPortSignalPtrsFcn(const SimStruct *S, int ip)
{
    ...
    if ((S)->portInfo.inputs[(ip)].signal.ptrs == NULL) {
        char *dialogMessage = (char *)malloc(sizeof(_ssBadLevel2ZeroDirectFeedMsg)
                                             + sizeof(_ssBadLevel2ZeroDirectFeedMsg2)
                                             + strlen(ssGetModelName(S))
                                             + strlen(ssGetPath(S)) +
                                             + 4*sizeof(char) + 1);
        if(dialogMessage != NULL){
            (void)sprintf(dialogMessage,_ssBadLevel2ZeroDirectFeedMsg,
                          ...
                          _ssBadLevel2ZeroDirectFeedMsg2);
            /* Stop the simulation and call _ssFatalError */
            ssSetStopRequested(S,1);
            _ssFatalError(S,dialogMessage);
            free(dialogMessage);
                                             ...
                                             int dirFeed)
{
    ...
    if (ip < 0 || ip >= numports) {
        char *dialogMessage =
            (char *)malloc(sizeof(_ssBadLevel2DirectFeedMsg)
                           +sizeof(_ssBadLevel2DirectFeedSolutionMsg)
                           + strlen(ssGetModelName(S))
                           + strlen(ssGetPath(S)) +
                           + 4*3*sizeof(char) + 1);
        char *solutionMessage =
            (char *)malloc(sizeof(_ssBadLevel2DirectFeedSolutionMsg)
                           + 4*sizeof(char) + 1);

        if (dialogMessage != NULL && solutionMessage != NULL) {
            (void)sprintf(dialogMessage,_ssBadLevel2DirectFeedMsg,
                          ssGetModelName(S), ssGetPath(S), ip+1, numports);
            (void)sprintf(solutionMessage, _ssBadLevel2DirectFeedSolutionMsg,
                          numports-1);
            (void)strcat(dialogMessage, solutionMessage);
            /* Stop the simulation and call _ssFatalError */
            ssSetStopRequested(S,1);
            _ssFatalError(S,dialogMessage);
            free(dialogMessage);
            free(solutionMessage);
        }
    } else {
 ...
 *	These adhear to the following format:
 *
...
#if defined(MDL_SLVRJACOBIAN)
  static void __mdlSlvrJacobian(SimStruct *S)
  {
      REQUIRE_LOCAL_STACK_FRAME;
      mdlSlvrJacobian(S);
  }
...
#if defined(MDL_SLVRJACOBIAN)
# define __mdlSlvrJacobian            mdlSlvrJacobian
#endif
...
#           if defined(MDL_SLVRJACOBIAN)
              ssSetmdlSlvrJacobian(S,__mdlSlvrJacobian);
#           endif
...
#         if S_FUNCTION_LEVEL == 2 && defined(MDL_INIT_SYSTEM_MATRICES)
	  case SS_CALL_MDL_INIT_SYSTEM_MATRICES:
            if (ssGetmdlInitSystemMatrices(S) != NULL) {
                ...
                ssSetErrorStatus(S,"Unexpected call to mdlInitSystemMatrices");
            }
            break;
#         endif
...
#           else
                ssSetErrorStatus(S,"Unexpected call to mdlOutputs "
                                 "in S-function that exports function-calls");
#           endif
            break;
...
#         if S_FUNCTION_LEVEL == 2 && defined(MDL_SLVRJACOBIAN)
	  case SS_CALL_MDL_SLVRJACOBIAN:
            if (ssGetmdlSlvrJacobian(S) != NULL) {
                mdlSlvrJacobian(S);
            } else {
                ssSetErrorStatus(S,"Unexpected call to mdlSlvrJacobian");
            }
            break;
#         endif
...
#         if S_FUNCTION_LEVEL == 2 && defined(MDL_CONSTRAINTS)
	  case SS_CALL_MDL_CONSTRAINTS:
            if (ssGetmdlConstraints(S) != NULL) {
                ...
                ssSetErrorStatus(S,"Unexpected call to mdlConstraints");
            }
            break;
#         endif
...
#         if S_FUNCTION_LEVEL == 2 && defined(MDL_SIM_STATE)
	  case SS_CALL_MDL_GET_SIM_STATE:
            if (ssGetSimStateCompliance(S) != USE_CUSTOM_SIM_STATE) {
                ssSetErrorStatus(S,"Unexpected call to mdlGetSimState");
            } else {
                mxArray** retVal;
	  ...
	  case SS_CALL_MDL_SET_SIM_STATE:
            if (ssGetSimStateCompliance(S) != USE_CUSTOM_SIM_STATE) {
                ssSetErrorStatus(S,"Unexpected call to mdlSetSimState");
            } else {
...
#         else
	  case SS_CALL_MDL_GET_SIM_STATE:
            ssSetErrorStatus(S,"Unexpected call to mdlGetSimState (MDL_SIM_STATE not defined)");
            break;
	  case SS_CALL_MDL_SET_SIM_STATE:
            ssSetErrorStatus(S,"Unexpected call to mdlSetSimState (MDL_SIM_STATE not defined)");
            break;
#         endif
